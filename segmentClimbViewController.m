/*
 segmentClimbViewController
 Shows a segments mac elevatiob, highest climb and overall gain.
 It also shows the segments elevation over time on a graph
 */

#import "segmentClimbViewController.h"

@interface segmentClimbViewController ()

@end

@implementation segmentClimbViewController
...
-(void)setUpGUI
{
    ...
    {
        maxClimb = [NSString stringWithFormat:@"%0.2fm", self.segment.hEle];
        start = [NSString stringWithFormat:@"%0.2fm", self.segment.hClimb];
        gain = [NSString stringWithFormat:@"%0.2fm", self.segment.gain];
    }
    else
    {
        maxClimb = [NSString stringWithFormat:@"%0.2fft", self.segment.hEle * 3.2808399];
        start = [NSString stringWithFormat:@"%0.2fft", self.segment.hClimb * 3.2808399];
        gain = [NSString stringWithFormat:@"%0.2fft", self.segment.gain * 3.2808399];
    }
...
- (int)sChart:(ShinobiChart *)chart numberOfDataPointsForSeriesAtIndex:(int)seriesIndex {
    ...
    return [self.segment.pointsArray count];
}
...
- (id<SChartData>)sChart:(ShinobiChart *)chart dataPointAtIndex:(int)dataIndex forSeriesAtIndex:(int)seriesIndex
{
    ...
    ridePoint *point = [self.segment.pointsArray objectAtIndex:dataIndex];
